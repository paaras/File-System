...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		od = ospfs_inode_data(dir_oi, (f_pos-2)*(OSPFS_DIRENTRY_SIZE));// Dir
		entry_oi = ospfs_inode(od->od_ino); // Inode
		
		if (od->od_ino == 0) // Blank directory?
		{
			f_pos++;
			continue;
		}
...
allocate_block(void)
{
	...
	uint32_t blocks = ospfs_super->os_nblocks;
	
	int j;
	for (j=0; j<blocks; j++)
	{
		if(bitvector_test(bitmap, j) == 1) // Free block
		{
			bitvector_clear(bitmap, j);
			return j;
		}
	}
...
add_block(ospfs_inode_t *oi)
{
	...
	{
		uint32_t allocBlockNum = allocate_block();
		if (allocBlockNum != 0)
		{
			oi->oi_direct[n] = allocBlockNum;
	...
	{
		uint32_t inAllocBlockNum = allocate_block();
		if (inAllocBlockNum != 0 && allocated[0] == 0 && n >= OSPFS_NDIRECT)
		{
			allocated[0] = inAllocBlockNum;
			oi->oi_indirect = inAllocBlockNum;
			oi->oi_size = ((n+1)*OSPFS_BLKSIZE);
		}
		else
		{
	...
	{
		uint32_t inAllocBlockNum2 = allocate_block();
		if (inAllocBlockNum2 != 0 && allocated[0] == 0 && n >= (OSPFS_NDIRECT+OSPFS_NINDIRECT))
		{
			allocated[0] = inAllocBlockNum2;
                        oi->oi_indirect2 = inAllocBlockNum2;
                        oi->oi_size = ((n+1)*OSPFS_BLKSIZE);
		}
		else
			if (allocated[0] != NULL)
...
remove_block(ospfs_inode_t *oi)
{
	...
	if (indir_index(n) == 0)
	{
		if (oi->oi_indirect)
		{
			...
			free_block(block[direct_index(n)-OSPFS_NDIRECT]);
			if (direct_index(n) < OSPFS_NDIRECT)
				free_block(oi->oi_indirect);
			else
				block[direct_index(n)-OSPFS_NDIRECT] = 0;
			oi->oi_size = (n-1)*OSPFS_BLKSIZE;
		}
		else
			return -EIO;
	...
	else if (indir2_index(n) == 0)
	{
		if (oi->oi_indirect2)
		{
			uint32_t *block = ospfs_block(oi->oi_indirect2);
                        free_block(block[(direct_index(n)-OSPFS_NDIRECT)%OSPFS_NINDIRECT]);
                        if (direct_index(n) < OSPFS_NDIRECT + OSPFS_NINDIRECT)
                                free_block(oi->oi_indirect2);
                        else
                                block[(direct_index(n)-OSPFS_NDIRECT)%OSPFS_NINDIRECT] = 0;
                        oi->oi_size = (n-1)*OSPFS_BLKSIZE;
		}
		else
			return -EIO;
	...
	{
		if (!oi->oi_direct[direct_index(n)])
			return -EIO;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	uint32_t dir = 0;
	ospfs_direntry_t *data;
	
	while (dir < dir_oi->oi_size)
	{
		data = ospfs_inode_data(dir_oi, dir);
		if(!data->od_ino)
			return data;
		dir = dir + OSPFS_DIRENTRY_SIZE;
	}
	
	uint32_t adding = add_block(dir_oi);
	
	if (adding < 0)
		return ERR_PTR(adding);
